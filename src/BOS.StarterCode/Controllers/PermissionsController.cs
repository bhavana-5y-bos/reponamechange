...
namespace BOS.StarterCode.Controllers
{
    ...
    public class PermissionsController : Controller
    {
        ...
        public async Task<ActionResult> FetchPermissions(string roleId, string roleName)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(roleId)) //Checking for non-null roleId. This is the for which the permissions are set
                {
                    ...
                    {
                        permittedModules = ownerPermissionsresponse.Permissions.Modules; //Assiging the BOS API response of flat-listed modules to the variable
                    ...
                    {
                        var moduleObj = allModules.Where(x => x.Id == module.ModuleId).FirstOrDefault(); //However, this works only at level-0, in the nested list, so we have a few custom methods that help us iterate through the N-Level down nested list
                        ...
                        else
                        {
                            ...
                            SetPermittedSubModules(module.ModuleId, ref allModules, ref permittedOperations);
                        }
                    }

                    model.ModuleOperations = allModules; //Finally, assigning the updated "allModules" nested-list to the model
        ...
        {
            if (modules.SelectMany(b => b.ChildModules).Where(x => x.Id == moduleId).FirstOrDefault() is Module moduleObj)
            {
            ...
            {
                var modulesList = modules.SelectMany(b => b.ChildModules).ToList();
        ...
        private void SetPermittedSubIModules(Guid moduleId, ref List<IModule> modules, ref List<IPermissionsOperation> permittedOperations)
        {
            ...
            {
                var modulesList = modules.SelectMany(b => b.ChildModules).ToList() as List<IModule>;
                if (modulesList.Count > 0)
                {
                    SetPermittedSubIModules(moduleId, ref modulesList, ref permittedOperations);
                }
